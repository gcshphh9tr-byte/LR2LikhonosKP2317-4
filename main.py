...
# # Тестовые cookies. В реальном приложении сюда можно подставить свои cookies.
# cookies = [
#     {
...
#     # Добавляем найденную угрозу в общий список.
#     threats.append({
#         "Cookie": cookie["name"],
#         "Категория": category,
#         "Угроза": threat,
...
#     # Проверяем одну cookie на разные виды угроз.
#     threats = []

#     # Угроза перехвата: cookie передаётся не только по HTTPS.
#     if not cookie["secure"]:
#         add_threat(threats, cookie, "Перехват",
#                    "Cookie может быть перехвачена при передаче по HTTP",
#                    "Критический", "Добавить флаг Secure")

#     # Угроза перехвата/CSRF: cookie отправляется при межсайтовых запросах.
...
#     # Угроза изменения: cookie можно перехватить и подменить.
...
#     # Угроза изменения: cookie живёт неопределённо долго или слишком долго.
#     month = 60 * 60 * 24 * 30
...
#     # Проверяем все cookies и создаём таблицу pandas.
#     all_threats = []
...
#     # Строим гистограмму во втором отдельном PNG-файле.
#     plt.figure(figsize=(7, 6))
#     plt.bar(counts.index, counts.values, color=colors)
#     plt.title("Гистограмма угроз")
#     plt.xlabel("Угроза доступа")
...
# def main():
#     # Запускаем анализ, выводим таблицу, сохраняем CSV и графики.
